...
export default function Edit({ badge }) {
    const currentIcon = badge.icon_path ? (badge.icon_path.startsWith('http') ? badge.icon_path : `/${badge.icon_path}`) : null;
    const [preview, setPreview] = useState(null);

    const { data, setData, post, processing, errors } = useForm({
        name: badge.name || '', requirement_description: badge.requirement_description || '',
        type: badge.type || 'general', category: badge.category || '',
        points: badge.points ?? 0, tier_level: badge.tier_level ?? 0, tier_target: badge.tier_target ?? 0,
        icon_path: null, remove_icon: false, _method: 'POST',
    });
    ...
    const submit = (e) => { e.preventDefault(); post(route('admin.badges.update', badge.id)); };

    const shownIcon = preview || (data.remove_icon ? null : currentIcon);

    return (
        <>
            <Head><title>Admin | Edit Lencana</title></Head>
            <div className="mx-auto w-full max-w-3xl">
                ...
                <h2 className="mb-6 border-b border-primary-10 pb-4 font-heading text-subtitle font-bold text-primary-100">Edit Lencana</h2>

                <form onSubmit={submit} className="flex flex-col gap-5">
                    <Input label="Nama Lencana" name="name" value={data.name} onChange={(e) => setData('name', e.target.value)} error={errors.name} required />

                    <Input type="textarea" rows={3} label="Deskripsi Syarat" name="requirement_description"
                        value={data.requirement_description} onChange={(e) => setData('requirement_description', e.target.value)} error={errors.requirement_description} />
                    ...
                    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                        ...
                        <Input label="Kategori Lencana" name="category" value={data.category} onChange={(e) => setData('category', e.target.value)} error={errors.category} />
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-dashed border-primary-30 bg-gray-10">
                                {shownIcon ? <img src={shownIcon} alt="Preview" className="h-full w-full object-contain p-2" /> : <span className="px-2 text-center text-micro italic text-gray-50">Tanpa icon</span>}
                            </div>
                            <div className="flex flex-col gap-2">
                                <input id="badge-icon" type="file" accept="image/*" onChange={handleFile} className="hidden" />
                                <Button variant="white" size="btn-sm" iconLeft={<FaUpload />} onClick={() => document.getElementById('badge-icon')?.click()}>Ganti File</Button>
                                {currentIcon && (
                                    <Checkbox id="remove_icon" name="remove_icon" label="Hapus icon saat ini"
                                        checked={data.remove_icon} onChange={(e) => { setData('remove_icon', e.target.checked); if (e.target.checked) { setData('icon_path', null); setPreview(null); } }} />
                                )}
                                <span className="text-micro text-gray-50">JPG, PNG, WEBP, SVG (Maks. 2MB).</span>
                            </div>
                        </div>
